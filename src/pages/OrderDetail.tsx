...
export default function OrderDetail() {
  ...
  const prepareExportData = (data: Lead[]) => {
    // 1. Identify all unique keys in extra_data across all sampled leads
    const extraKeys = new Set<string>();
    data.forEach(l => {
      if (l.extra_data && typeof l.extra_data === 'object') {
        Object.keys(l.extra_data).forEach(k => extraKeys.add(k));
      }
    });
    const sortedExtras = Array.from(extraKeys).sort();

    // 2. Map leads to flat objects with human-friendly headers
    return data.map(l => {
      const row: any = {
      ...
      // Add all detected extra columns
      sortedExtras.forEach(key => {
        row[key] = (l.extra_data as any)?.[key] ?? "";
      });
  ...
  const exportCSV = () => {
    const exportData = prepareExportData(filtered);
