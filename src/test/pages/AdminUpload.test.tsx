import { describe, it, expect, vi, beforeEach } from "vitest";
import { render, screen, waitFor } from "@testing-library/react";
import { BrowserRouter } from "react-router-dom";
import AdminUpload from "@/pages/AdminUpload";

const mockToastSuccess = vi.fn();
const mockToastError = vi.fn();

vi.mock("sonner", () => ({
    toast: {
        success: (msg: string) => mockToastSuccess(msg),
        error: (msg: string) => mockToastError(msg),
    },
}));

const mockFrom = vi.fn();

vi.mock("@/integrations/supabase/client", () => ({
    supabase: {
        from: (...args: any[]) => mockFrom(...args),
        channel: () => ({
            on: () => ({
                subscribe: () => ({ unsubscribe: vi.fn() }),
            }),
        }),
        removeChannel: vi.fn(),
    },
}));

vi.mock("@/contexts/AuthContext", () => ({
    useAuth: () => ({
        profileId: "admin-123",
    }),
}));

vi.mock("@/components/AdminLayout", () => ({
    AdminLayout: ({ children, title }: { children: React.ReactNode; title: string }) => (
        <div data-testid="admin-layout">
            <h1>{title}</h1>
            {children}
        </div>
    ),
}));

// ── helpers ──────────────────────────────────────────────────────────────────

function setupMockFrom(overrides: Record<string, any> = {}) {
    mockFrom.mockImplementation((table: string) => {
        if (overrides[table]) return overrides[table];

        if (table === "orders") {
            return {
                select: () => ({
                    in: () => ({
                        order: () => Promise.resolve({
                            data: [
                                {
                                    id: "order-1",
                                    title: "Test Order",
                                    total_leads_ordered: 1000,
                                    leads_per_day: 50,
                                    client_id: "client-1",
                                    vendor_id: "vendor-1",
                                },
                            ],
                            error: null,
                        }),
                    }),
                }),
            };
        }
        if (table === "profiles") {
            return {
                select: () => ({
                    in: () => Promise.resolve({
                        data: [{ id: "client-1", name: "John Doe", email: "john@example.com" }],
                        error: null,
                    }),
                }),
            };
        }
        return {
            select: () => Promise.resolve({ data: [], error: null }),
        };
    });
}

// ── tests ─────────────────────────────────────────────────────────────────────

describe("AdminUpload Page", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });

    it("should render upload page with order selection", async () => {
        setupMockFrom();

        render(
            <BrowserRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
                <AdminUpload />
            </BrowserRouter>
        );

        await waitFor(() => {
            expect(screen.getByTestId("admin-layout")).toBeInTheDocument();
            expect(screen.getByText(/upload leads/i)).toBeInTheDocument();
            expect(screen.getByText(/select order to deliver against/i)).toBeInTheDocument();
        });
    });

    it("should show no active orders message when none exist", async () => {
        setupMockFrom({
            orders: {
                select: () => ({
                    in: () => ({
                        order: () => Promise.resolve({ data: [], error: null }),
                    }),
                }),
            },
        });

        render(
            <BrowserRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
                <AdminUpload />
            </BrowserRouter>
        );

        await waitFor(() => {
            expect(screen.getByText(/no active orders/i)).toBeInTheDocument();
        });
    });

    it("should show progress bar when order is selected", async () => {
        setupMockFrom({
            leads: {
                select: () => ({
                    in: () => Promise.resolve({ data: [], error: null }),
                    eq: () => ({
                        eq: () => Promise.resolve({ data: [], count: 0, error: null }),
                    }),
                }),
            },
        });

        render(
            <BrowserRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
                <AdminUpload />
            </BrowserRouter>
        );

        await waitFor(() => {
            expect(screen.getByText(/select an order/i)).toBeInTheDocument();
        });
    });

    it("should handle file upload area rendering", async () => {
        setupMockFrom();

        render(
            <BrowserRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
                <AdminUpload />
            </BrowserRouter>
        );

        await waitFor(() => {
            expect(screen.getByText(/select an order/i)).toBeInTheDocument();
        });
    });

    // ── new: verify extra_data logic ────────────────────────────────────────
    it("should separate core fields and extra columns correctly", () => {
        // Simulate the transformation logic directly (pure function behaviour)
        const rawRow: Record<string, string> = {
            "First Name":    "Alice",
            "Last Name":     "Smith",
            "Email Address": "alice@example.com",
            "Phone Number":  "555-1234",
            "Bank":          "Chase",
            "Address":       "123 Main St",
            "City":          "Austin",
            "State":         "TX",
            "Zip":           "78701",
            // Extra columns — these must NOT be dropped
            "Signup Date":   "2024-01-15",
            "IP Address":    "192.168.1.1",
            "Loan Amount":   "5000",
            "Next Paydate":  "2024-02-01",
        };

        // Replicate the auto-mapping the component would produce
        const mapping: Record<string, string> = {
            FirstName: "First Name",
            LastName:  "Last Name",
            Email:     "Email Address",
            Phone:     "Phone Number",
            Bank:      "Bank",
            Address:   "Address",
            City:      "City",
            State:     "State",
            Zip:       "Zip",
        };

        const CORE_FIELDS = ["Bank", "FirstName", "LastName", "Address", "City", "State", "Zip", "Phone", "Email"] as const;
        const usedSourceColumns = new Set(Object.values(mapping));

        // Apply the same logic as mappedData builder in AdminUpload
        const result: Record<string, any> = {};
        for (const field of CORE_FIELDS) {
            const src = mapping[field];
            result[field] = src ? (rawRow[src] ?? "") : "";
        }
        const extra: Record<string, string> = {};
        for (const header of Object.keys(rawRow)) {
            if (!usedSourceColumns.has(header)) {
                extra[header] = rawRow[header] ?? "";
            }
        }
        result.extra_data = extra;

        // Core fields mapped correctly
        expect(result.FirstName).toBe("Alice");
        expect(result.LastName).toBe("Smith");
        expect(result.Email).toBe("alice@example.com");
        expect(result.Bank).toBe("Chase");

        // Extra columns preserved — not dropped
        expect(result.extra_data).toBeDefined();
        expect(result.extra_data["Signup Date"]).toBe("2024-01-15");
        expect(result.extra_data["IP Address"]).toBe("192.168.1.1");
        expect(result.extra_data["Loan Amount"]).toBe("5000");
        expect(result.extra_data["Next Paydate"]).toBe("2024-02-01");

        // No extra data leaking into core field slots
        expect(result["Signup Date"]).toBeUndefined();
        expect(result["IP Address"]).toBeUndefined();
    });
});
